...
from typing import List, Tuple, Optional, Dict, Any, Literal, Callable, Union, Sequence, Mapping
    ...
    def attention(self, x: torch.Tensor):
        ...
        if self.capture_read_null_attention:
            out, weights = self.attn(
                x, x, x, need_weights=True, average_attn_weights=False, attn_mask=mask
            )
            # weights: [B,H,T,T]. READ_NULL is always the final token.
            with torch.no_grad():
