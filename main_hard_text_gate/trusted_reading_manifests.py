"""Load high-trust natural-text manifests for TextCaps and COCO-SPRIGHT.

These helpers deliberately expose only positive readable evidence.  Absence from
one of the derived OCR manifests is *unknown*, never a ``<text><null>`` label.
"""
...
def _require_high_trust_textcaps(payload: Mapping[str, Any], path: Path) -> None:
    settings = payload.get("settings") or {}
    semantics = payload.get("semantics") or {}
    confidence = float(settings.get("min_confidence", 0.0))
    max_detections = int(settings.get("max_raw_accepted_detections", 10**9))
    rotations = {int(value) for value in settings.get("rotations_expected") or []}
    if confidence < 0.999:
        raise ValueError(f"TextCaps manifest is not high-trust: min_confidence={confidence} in {path}")
    if max_detections > 3:
        raise ValueError(f"TextCaps manifest permits dense rows: max_detections={max_detections} in {path}")
    if rotations and rotations != {0}:
        raise ValueError(f"TextCaps training manifest must be upright-only, got rotations={sorted(rotations)} in {path}")
    if semantics.get("ocr_absence_creates_null_target") is not False:
        raise ValueError(f"TextCaps manifest must not convert OCR absence into null supervision: {path}")
    if float(semantics.get("present", -1.0)) != 1.0 or float(semantics.get("readable", -1.0)) != 1.0:
        raise ValueError(f"TextCaps trusted entries must be present=1/readable=1: {path}")
...
def load_textcaps_reading_manifest(path: Optional[Path], split: str) -> Dict[str, Dict[str, Any]]:
    if path is None:
        return {}
    path = Path(path)
    if not path.is_file():
        raise FileNotFoundError(f"Trusted TextCaps reading manifest does not exist: {path}")
    payload = _read_json(path)
    _require_high_trust_textcaps(payload, path)
    rows = _as_entries(payload, "entries")
    wanted = str(split).casefold()
    out: Dict[str, Dict[str, Any]] = {}
    for row in rows:
        row_split = str(row.get("split") or "").casefold()
        if row_split and row_split != wanted:
            continue
        key = normalize_image_key(row.get("image"))
        if not key:
            continue
        targets = [dict(target) for target in row.get("targets") or [] if isinstance(target, Mapping)]
        if not targets:
            continue
        _validate_reading_targets(targets, source=f"TextCaps manifest row {key}")
        row["targets"] = targets
        if key in out:
            raise ValueError(f"Duplicate TextCaps reading-manifest key after normalization: {key}")
        out[key] = row
    return out
...
def caption_mentions_target(caption: str, texts: Sequence[str]) -> bool:
    words = {
        token
        for token in "".join(ch.casefold() if ch.isalnum() else " " for ch in str(caption)).split()
        if token
    }
    for text in texts:
        normalized = [
            token
            for token in "".join(ch.casefold() if ch.isalnum() else " " for ch in str(text)).split()
            if token
        ]
        if normalized and all(token in words for token in normalized):
            return True
    return False
