...
By default the original visual and text towers remain frozen. With --all_weights_grad, every model parameter is trainable and the run becomes a fully instrumented whole-CLIP continuation. The detachable content correction
...
# -----------------------------------------------------------------------------
# Whole-model optimizer, finite-batch handling, and gradient instrumentation
# -----------------------------------------------------------------------------


def _all_weights_no_decay(name: str, parameter: torch.nn.Parameter) -> bool:
    """Conservative exclusions for decoupled weight decay.

    For ``W = r * normalize(theta)``, ordinary weight decay belongs on ``r``:
    it changes the represented row norm. Scaling ``theta`` is a gauge change
    cancelled by normalization, so ``.theta`` is excluded from decay.
    """
    lower = name.lower()
    leaf = lower.rsplit(".", 1)[-1]
    if parameter.ndim <= 1:
        return True
    if leaf == "bias" or lower.endswith(".theta"):
        return True
    if any(token in lower for token in (
        "layernorm", ".ln_", ".ln.", "positional_embedding", "class_embedding",
        "logit_scale",
        "tap_logits", "embedding", "glyph_bias_beta", "auto_read_scale",
        "null_abstain_weight", "read_calibration_scale", "register_gate",
    )):
        return True
    return False


def optimizer_for_all_weights(model: torch.nn.Module, args: argparse.Namespace):
    decay, no_decay, router = [], [], []
    decay_names, no_decay_names, router_names = [], [], []
    for name, parameter in model.named_parameters():
        if not parameter.requires_grad:
            continue
        if name.startswith("read_implant.trust_router.") and args.router_aux_enabled:
            router.append(parameter)
            router_names.append(name)
        elif _all_weights_no_decay(name, parameter):
    ...
    groups = []
    if decay:
        groups.append({"params": decay, "lr": args.all_weights_lr,
                       "weight_decay": args.all_weights_weight_decay,
                       "group_name": "decay"})
    if no_decay:
        groups.append({"params": no_decay, "lr": args.all_weights_lr,
                       "weight_decay": 0.0, "group_name": "no_decay"})
    if router:
        groups.append({"params": router, "lr": args.router_aux_all_weights_warmup_start_lr,
                       "weight_decay": 0.0, "group_name": "router"})
    try:
        from adabelief_pytorch import AdaBelief
    except ImportError as exc:
        raise ImportError(
            "--all_weights_grad requires adabelief-pytorch. Install with: "
            "pip install adabelief-pytorch"
        ) from exc
    optimizer = AdaBelief(
        groups,
        lr=args.all_weights_lr,
        eps=args.all_weights_eps,
        betas=(args.all_weights_beta1, args.all_weights_beta2),
        weight_decay=0.0,
        weight_decouple=True,
        rectify=True,
        print_change_log=False,
    )
    return optimizer, {"decay": decay_names, "no_decay": no_decay_names, "router": router_names}

def all_weights_absolute_lr(
    optimizer: torch.optim.Optimizer,
    step: int,
    total_steps: int,
    args: argparse.Namespace,
) -> float:
    warmup = max(1, min(args.sched_warmup_batches, total_steps))

    def scheduled(start_lr: float, peak_lr: float, floor_lr: float) -> float:
        if step < warmup:
            fraction = (step + 1) / warmup
            return start_lr + fraction * (peak_lr - start_lr)
        progress = (step - warmup) / max(1, total_steps - warmup - 1)
        progress = min(max(progress, 0.0), 1.0)
        cosine = 0.5 * (1.0 + math.cos(math.pi * progress))
        return floor_lr + cosine * (peak_lr - floor_lr)

    main_lr = scheduled(
        args.sched_warmup_start_lr, args.sched_peak_lr, args.sched_floor_lr
    )
    router_lr = scheduled(
        args.router_aux_all_weights_warmup_start_lr,
        args.router_aux_all_weights_peak_lr,
        args.router_aux_all_weights_floor_lr,
    )
    for group in optimizer.param_groups:
        group["lr"] = router_lr if group.get("group_name") == "router" else main_lr
    return main_lr


def _gradient_category(name: str) -> str:
    lower = name.lower()
    if lower == "visual.conv1.weight" or lower.endswith("visual.conv1.weight"):
        return "conv1"
    if "positional_embedding" in lower:
        return "pos_embed"
    if "class_embedding" in lower:
        return "class_embed"
    if "logit_scale" in lower:
        return "logit_scale"
    if lower.endswith(".theta"):
        return "gmp_theta"
    if lower.endswith(".r"):
        return "gmp_r"
    if "attn" in lower and lower.endswith("bias"):
        return "attn_bias"
    if "attn" in lower and lower.endswith("weight"):
        return "attn_weight"
    if ".mlp." in lower and lower.endswith("bias"):
        return "mlp_bias"
    if any(token in lower for token in ("layernorm", ".ln_", ".ln.")):
        return "layernorm"
    if lower.endswith("bias"):
        return "other_bias"
    if lower.startswith("read_implant.") or lower in {
        "hard_text_embedding", "null_text_embedding"
    }:
        return "implant"
    if lower.startswith("visual."):
        return "visual_other"
    if lower.startswith("transformer.") or lower.startswith("token_embedding") \
...
def nonfinite_gradient_report(
    model: torch.nn.Module, *, max_parameters: int = 32
) -> List[Dict[str, Any]]:
    """Describe parameters carrying NaN/Inf gradients before a dropped step.

    This is diagnostic only.  It deliberately runs *before* ``zero_grad`` so a
    smoke test can identify the exact tensor(s) that poisoned an all-weights
    update instead of reporting only a generic non-finite window.
    """
    rows: List[Dict[str, Any]] = []
    for name, parameter in model.named_parameters():
        grad = parameter.grad
        ...
        rows.append({
            ...
            "finite_abs_max": (
                float(finite_values.detach().float().abs().max().cpu())
...
def lower_grad_scaler_after_drop(scaler: torch.amp.GradScaler) -> None:
    if not scaler.is_enabled():
        return
    current = float(scaler.get_scale())
    try:
        scaler.update(new_scale=max(1.0, current * 0.5))
    except TypeError:
        scaler.update()


def capture_gradient_diagnostics(
    model: torch.nn.Module,
    phase: str,
    epoch: int,
    global_step: int,
    ...
    layer_stats: Dict[Tuple[str, str], Dict[str, float]] = {}
    conv1_grad = None
    pos_embed_grad = None
    for name, parameter in model.named_parameters():
        grad = parameter.grad
        ...
        stat = categories.setdefault(category, {
            "sum_sq": 0.0, "parameter_sum_sq": 0.0,
            "numel": 0.0, "max_abs": 0.0,
            "nonzero": 0.0, "tensor_count": 0.0,
        })
        sum_sq = float(g.square().sum().cpu())
        max_abs = float(g.abs().max().cpu()) if g.numel() else 0.0
        stat["sum_sq"] += sum_sq
        stat["parameter_sum_sq"] += float(parameter.detach().float().square().sum().cpu())
        stat["numel"] += g.numel()
        stat["max_abs"] = max(stat["max_abs"], max_abs)
        stat["nonzero"] += float(torch.count_nonzero(g).cpu())
        stat["tensor_count"] += 1.0

        visual_match = re.search(r"visual\.transformer\.resblocks\.(\d+)", name)
        text_match = re.search(r"(?<!visual\.)transformer\.resblocks\.(\d+)", name)
        if visual_match:
            layer_name = f"visual_B{int(visual_match.group(1)):02d}"
        elif text_match:
            layer_name = f"text_B{int(text_match.group(1)):02d}"
        elif name.startswith("visual."):
            layer_name = "visual_stem_or_head"
        elif name.startswith("read_implant."):
            layer_name = "implant"
        else:
            layer_name = "other"
        layer_stat = layer_stats.setdefault((layer_name, category), {
            "sum_sq": 0.0, "numel": 0.0, "max_abs": 0.0,
        })
        layer_stat["sum_sq"] += sum_sq
        layer_stat["numel"] += g.numel()
        layer_stat["max_abs"] = max(layer_stat["max_abs"], max_abs)

        parameter_rows.append({
            "phase": phase, "epoch": epoch, "step": global_step,
            "name": name, "category": category,
            "grad_l2": math.sqrt(sum_sq),
            "grad_rms": math.sqrt(sum_sq / max(1, g.numel())),
            "grad_max_abs": max_abs,
            "parameter_l2": float(parameter.detach().float().norm().cpu()),
            "numel": int(g.numel()),
        })
        if category == "conv1" and g.ndim == 4:
            conv1_grad = g
        if name == "visual.positional_embedding" and g.ndim == 2:
            pos_embed_grad = g

        if name.endswith("attn.in_proj_weight") and g.ndim == 2 and g.shape[0] % 3 == 0:
            embed_dim = g.shape[1]
            module_name = name.rsplit(".in_proj_weight", 1)[0]
            try:
                attn_module = model.get_submodule(module_name)
                num_heads = int(attn_module.num_heads)
            except Exception:
                num_heads = 1
            if embed_dim % num_heads == 0:
                head_dim = embed_dim // num_heads
                qkv = g.view(3, num_heads, head_dim, embed_dim)
                for qkv_index, qkv_name in enumerate(("Q", "K", "V")):
                    for head in range(num_heads):
                        hg = qkv[qkv_index, head]
                        append_csv(out_dir / "attention_head_grad_log.csv", {
                            "phase": phase, "epoch": epoch, "step": global_step,
                            "module": module_name, "qkv": qkv_name, "head": head,
                            "grad_l2": float(hg.norm().cpu()),
                            "grad_rms": float(hg.square().mean().sqrt().cpu()),
                            "grad_max_abs": float(hg.abs().max().cpu()),
                        })

    summary: Dict[str, float] = {}
    for category, stat in sorted(categories.items()):
        row = {
            "phase": phase, "epoch": epoch, "step": global_step, "lr": lr,
            "category": category,
            "grad_l2": math.sqrt(stat["sum_sq"]),
            "grad_rms": math.sqrt(stat["sum_sq"] / max(1.0, stat["numel"])),
            "grad_max_abs": stat["max_abs"],
            "parameter_l2": math.sqrt(stat["parameter_sum_sq"]),
            "grad_to_parameter_l2": math.sqrt(stat["sum_sq"]) / max(1.0e-30, math.sqrt(stat["parameter_sum_sq"])),
            "nonzero_fraction": stat["nonzero"] / max(1.0, stat["numel"]),
            "tensor_count": int(stat["tensor_count"]),
            "numel": int(stat["numel"]),
        }
        append_csv(out_dir / "grad_category_log.csv", row)
        summary[f"grad_{category}_rms"] = row["grad_rms"]
        summary[f"grad_{category}_max"] = row["grad_max_abs"]

    for (layer_name, category), stat in sorted(layer_stats.items()):
        append_csv(out_dir / "layer_grad_log.csv", {
            "phase": phase, "epoch": epoch, "step": global_step,
            "layer": layer_name, "category": category,
            "grad_l2": math.sqrt(stat["sum_sq"]),
            "grad_rms": math.sqrt(stat["sum_sq"] / max(1.0, stat["numel"])),
            "grad_max_abs": stat["max_abs"], "numel": int(stat["numel"]),
        })

    parameter_rows.sort(key=lambda row: row["grad_rms"], reverse=True)
    for rank, row in enumerate(parameter_rows[:max(1, topk)], start=1):
        append_csv(out_dir / "parameter_grad_topk.csv", {"rank": rank, **row})

    if conv1_grad is not None:
        flat = conv1_grad.flatten(1)
        channel_l2 = flat.norm(dim=1).cpu().numpy()
        channel_rms = flat.square().mean(dim=1).sqrt().cpu().numpy()
        channel_max = flat.abs().amax(dim=1).cpu().numpy()
        for channel, (l2, rms, max_abs) in enumerate(
            zip(channel_l2, channel_rms, channel_max)
        ):
            append_csv(out_dir / "conv1_channel_grad_log.csv", {
                "phase": phase, "epoch": epoch, "step": global_step,
                "channel": channel, "grad_l2": float(l2),
                "grad_rms": float(rms), "grad_max_abs": float(max_abs),
            })
        summary["conv1_channel_max_rms"] = float(channel_rms.max())
        summary["conv1_channel_median_rms"] = float(np.median(channel_rms))
        summary["conv1_channel_divergence_ratio"] = float(
            channel_rms.max() / max(1.0e-30, np.median(channel_rms))
        )
    if pos_embed_grad is not None:
        token_rms = pos_embed_grad.square().mean(dim=1).sqrt().cpu().numpy()
        token_l2 = pos_embed_grad.norm(dim=1).cpu().numpy()
        for token_index, (rms, l2) in enumerate(zip(token_rms, token_l2)):
            append_csv(out_dir / "pos_embed_token_grad_log.csv", {
                "phase": phase, "epoch": epoch, "step": global_step,
                "token_index": token_index,
                "token_type": "CLS" if token_index == 0 else "patch",
                "grad_rms": float(rms), "grad_l2": float(l2),
            })
    return summary
...
def save_training_diagnostic_plots(out_dir: Path) -> None:
    plot_dir = out_dir / "plots"
    plot_dir.mkdir(parents=True, exist_ok=True)

    rows = _read_csv_rows(out_dir / "grad_category_log.csv")
    if rows:
        by_category: Dict[str, List[Tuple[int, float]]] = {}
        for row in rows:
            by_category.setdefault(row["category"], []).append(
                (int(float(row["step"])), float(row["grad_rms"]))
            )
        fig, ax = plt.subplots(figsize=(13, 7))
        for category, values in sorted(by_category.items()):
            values.sort()
            ax.plot([x for x, _ in values], [y for _, y in values], label=category)
        ax.set_yscale("log")
        ax.set_xlabel("optimizer step")
        ax.set_ylabel("unscaled gradient RMS")
        ax.set_title("Whole-model unscaled gradient RMS by component")
        ax.grid(True, alpha=0.25)
        ax.legend(ncol=3, fontsize=8)
        fig.tight_layout()
        fig.savefig(plot_dir / "gradient_rms_by_component.png", dpi=180)
        plt.close(fig)

    conv_rows = _read_csv_rows(out_dir / "conv1_channel_grad_log.csv")
    if conv_rows:
        ...
        for row in conv_rows:
            ...
            rms = float(row["grad_rms"])
            totals[channel] = totals.get(channel, 0.0) + rms * rms
            traces.setdefault(channel, []).append((step, rms))
        top = sorted(totals, key=totals.get, reverse=True)[:20]
        fig, ax = plt.subplots(figsize=(13, 7))
        for channel in top[:12]:
            values = sorted(traces[channel])
            ax.plot([x for x, _ in values], [y for _, y in values], label=f"ch{channel}")
        ax.set_yscale("log")
        ax.set_xlabel("optimizer step")
        ax.set_ylabel("conv1 channel gradient RMS")
        ax.set_title("Most divergent conv1 channels")
        ax.grid(True, alpha=0.25)
        ax.legend(ncol=3, fontsize=8)
        fig.tight_layout()
        fig.savefig(plot_dir / "conv1_top_channel_trajectories.png", dpi=180)
        plt.close(fig)

        fig, ax = plt.subplots(figsize=(12, 6))
        scores = [math.sqrt(totals[channel]) for channel in top]
        ax.bar([str(channel) for channel in top], scores)
        ax.set_yscale("log")
        ax.set_xlabel("conv1 output channel")
        ax.set_ylabel("root-sum-square gradient RMS across logged steps")
        ax.set_title("Top conv1 channels by cumulative gradient")
        fig.tight_layout()
        fig.savefig(plot_dir / "conv1_top_channels_cumulative.png", dpi=180)
        plt.close(fig)

    layer_rows = _read_csv_rows(out_dir / "layer_grad_log.csv")
    if layer_rows:
        visual_rows = [row for row in layer_rows if row["layer"].startswith("visual_B")]
        blocks = sorted({row["layer"] for row in visual_rows})
        categories = sorted({row["category"] for row in visual_rows})
        if blocks and categories:
            block_index = {name: i for i, name in enumerate(blocks)}
            cat_index = {name: i for i, name in enumerate(categories)}
            accum = np.zeros((len(categories), len(blocks)), dtype=np.float64)
            count = np.zeros_like(accum)
            for row in visual_rows:
                i = cat_index[row["category"]]
                j = block_index[row["layer"]]
                value = float(row["grad_rms"])
                accum[i, j] += value * value
                count[i, j] += 1
            heat = np.sqrt(accum / np.maximum(count, 1.0))
            fig, ax = plt.subplots(figsize=(16, max(5, 0.45 * len(categories))))
            image = ax.imshow(np.log10(np.maximum(heat, 1.0e-30)), aspect="auto")
            ax.set_xticks(range(len(blocks)), [name.replace("visual_", "") for name in blocks], rotation=45)
            ax.set_yticks(range(len(categories)), categories)
            ax.set_title("Visual-block gradient RMS (log10, RMS over logged steps)")
            fig.colorbar(image, ax=ax, label="log10 gradient RMS")
            fig.tight_layout()
            fig.savefig(plot_dir / "visual_block_gradient_heatmap.png", dpi=180)
            plt.close(fig)

    pos_rows = _read_csv_rows(out_dir / "pos_embed_token_grad_log.csv")
    if pos_rows:
        totals: Dict[int, float] = {}
        counts: Dict[int, int] = {}
        for row in pos_rows:
            ...
            value = float(row["grad_rms"])
            totals[token] = totals.get(token, 0.0) + value * value
            counts[token] = counts.get(token, 0) + 1
        patch_tokens = sorted(token for token in totals if token > 0)
        side = int(round(math.sqrt(len(patch_tokens))))
        if side * side == len(patch_tokens):
            values = [math.sqrt(totals[token] / counts[token]) for token in patch_tokens]
            grid = np.asarray(values).reshape(side, side)
            fig, ax = plt.subplots(figsize=(7, 6))
            image = ax.imshow(np.log10(np.maximum(grid, 1.0e-30)))
            cls_value = math.sqrt(totals.get(0, 0.0) / max(1, counts.get(0, 1)))
            ax.set_title(f"Positional-embedding patch gradients (log10 RMS)\nCLS RMS={cls_value:.3e}")
            fig.colorbar(image, ax=ax, label="log10 gradient RMS")
            fig.tight_layout()
            fig.savefig(plot_dir / "pos_embed_patch_gradient_heatmap.png", dpi=180)
            plt.close(fig)

    train_rows = _read_csv_rows(out_dir / "training_log.csv")
    if train_rows:
        steps, losses, lrs, scales = [], [], [], []
        for row in train_rows:
            try:
                steps.append(int(float(row["step"])))
                losses.append(float(row["loss_total"]))
                lrs.append(float(row.get("lr", "nan")))
                scales.append(float(row.get("logit_scale_exp", "nan")))
            except (KeyError, ValueError):
                continue
        if steps:
            fig, ax = plt.subplots(figsize=(13, 6))
            ax.plot(steps, losses)
            ax.set_xlabel("microbatch step")
            ax.set_ylabel("training loss")
            ax.set_title("Training loss")
            ax.grid(True, alpha=0.25)
            fig.tight_layout()
            fig.savefig(plot_dir / "training_loss.png", dpi=180)
            plt.close(fig)
        if steps and any(math.isfinite(x) for x in lrs):
            fig, ax = plt.subplots(figsize=(13, 6))
            ax.plot(steps, lrs)
            ax.set_xlabel("microbatch step")
            ax.set_ylabel("learning rate")
            ax.set_title("Warmup-cosine learning rate")
            ax.grid(True, alpha=0.25)
            fig.tight_layout()
            fig.savefig(plot_dir / "learning_rate.png", dpi=180)
            plt.close(fig)
        if steps and any(math.isfinite(x) for x in scales):
            fig, ax = plt.subplots(figsize=(13, 6))
            ax.plot(steps, scales)
            ax.axhline(100.0, linestyle="--", linewidth=1)
            ax.set_xlabel("microbatch step")
            ax.set_ylabel("exp(logit_scale)")
            ax.set_title("CLIP logit scale")
            ax.grid(True, alpha=0.25)
            fig.tight_layout()
            fig.savefig(plot_dir / "logit_scale.png", dpi=180)
            plt.close(fig)


def save_full_model_state(model: torch.nn.Module, path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
            "state_dict": {
            ...
            "metadata": {
                ...
                "format": "gmp_anytext_full_state_v2",
            },
        },
        path,
    )
...
def load_full_model_checkpoint(model: torch.nn.Module, path: Path) -> None:
    """Restore a complete merged checkpoint with architecture validation.

    ``export_merged`` stores model tensors below ``state_dict`` alongside ordinary
    metadata.  Passing that outer wrapper directly to ``load_state_dict`` makes
    every real model parameter appear missing, so always normalize the checkpoint
    through ``_checkpoint_state_and_architecture`` first.
    """
    loaded = torch_load_trusted(path)
    state, checkpoint_architecture = _checkpoint_state_and_architecture(loaded)
    if state is None:
        raise RuntimeError(f"Full model checkpoint contains no state_dict: {path}")
    ...
    if (
        ...
        raise ValueError(
            "Cannot restore a full checkpoint built for a different "
            "read-attention architecture: "
            f"model={model_architecture!r}, "
            f"checkpoint={checkpoint_architecture!r}, path={path}"
        )
...
def train(args: argparse.Namespace) -> None:
    ...
    for phase in PHASES[start_index:]:
        ...
        trainable_names = set_trainable_phase(
            ...
            all_weights_grad=args.all_weights_grad,
        )
        ...
        if args.all_weights_grad:
            optimizer, decay_manifest = optimizer_for_all_weights(model, args)
            write_json(out_dir / f"phase_{phase}_weight_decay_groups.json", decay_manifest)
        else:
            optimizer = optimizer_for_phase(model, args, phase)
        scaler = precision_make_grad_scaler(device, args.amp_dtype)
        total_steps = epochs * args.steps_per_epoch
        total_update_steps = math.ceil(total_steps / max(1, args.grad_accum_steps))
        local_step = 0
        phase_update_step = 0
        ...
        for epoch in range(1, epochs + 1):
            ...
            dropped_nonfinite_loss = 0
            dropped_nonfinite_grad = 0
            ...
            for step_in_epoch in range(1, args.steps_per_epoch + 1):
                ...
                if args.all_weights_grad:
                    current_lr = all_weights_absolute_lr(
                        optimizer, phase_update_step, total_update_steps, args
                    )
                else:
                ...
                with torch.no_grad():
                    if hasattr(model, "logit_scale"):
                        model.logit_scale.clamp_(max=math.log(args.logit_scale_max))
                will_step = (
                    accumulated_microbatches + 1 >= args.grad_accum_steps
                    or step_in_epoch == args.steps_per_epoch
                )
                next_optimizer_step = phase_update_step + 1
                do_precision_log = will_step and precision_diag.should_log(
                    next_optimizer_step,
                ...
                # Keep precision hooks scoped to the ordinary task forward. The
                # router auxiliary performs a second no-grad full-model pass; running
                # it after the hook context prevents those aux activations from
                # replacing the task activations in precision diagnostics.
                ...
                with amp_context(device, args.amp_dtype):
                    ...
                    if (
                        args.router_aux_enabled and phase == "1c" and will_step
                ...
                if not finite_loss:
                    dropped_nonfinite_loss += 1
                    optimizer.zero_grad(set_to_none=True)
                    accumulated_microbatches = 0
                    print(f"[drop] non-finite loss at {phase} e{epoch} s{step_in_epoch}")
                    continue
                ...
                should_step = will_step
                if should_step:
                    scaler.unscale_(optimizer)
                    if args.router_aux_enabled and phase == "1c":
                        metrics["router_grad_preclip_norm"] = router_auxiliary.router_grad_norm(model)
                    if not gradients_are_finite(model):
                        dropped_nonfinite_grad += 1
                        bad_grads = nonfinite_gradient_report(model)
                        report_path = (
                            out_dir
                            / f"nonfinite_grad_{phase}_e{epoch:03d}_s{step_in_epoch:05d}.json"
                        )
                        write_json(
                            report_path,
                            {
                                ...
                                "optimizer_step_attempt": int(next_optimizer_step),
                                "amp_dtype": str(args.amp_dtype),
                                "all_weights_grad": bool(args.all_weights_grad),
                                "parameters": bad_grads,
                            },
                        )
                        preview = ", ".join(
                            f"{row['name']}[nan={row['nan']},+inf={row['posinf']},-inf={row['neginf']}]"
                            for row in bad_grads[:4]
                        ) or "<no offending parameter located>"
                        optimizer.zero_grad(set_to_none=True)
                        accumulated_microbatches = 0
                        lower_grad_scaler_after_drop(scaler)
                        print(
                            f"[drop] non-finite gradient window at {phase} e{epoch} s{step_in_epoch}; "
                            f"offending={preview}; report={report_path}"
                        )
                        continue
                    ...
                    if args.all_weights_grad and (phase_update_step + 1) % args.grad_log_every == 0:
                        latest_grad_metrics = capture_gradient_diagnostics(
                            model, phase, epoch, phase_update_step + 1, current_lr,
                            out_dir, args.grad_topk,
                        )
                    if not args.all_weights_grad:
                        torch.nn.utils.clip_grad_norm_(
                            [parameter for parameter in model.parameters() if parameter.requires_grad],
                            args.grad_clip,
                        )
                    elif args.router_aux_enabled and phase == "1c":
                        # Preserve the original no-global-clip all-weights optimizer, but
                        # keep the tiny router itself on the A4-style 1.0 leash.
                        torch.nn.utils.clip_grad_norm_(
                            list(model.read_implant.trust_router.parameters()),
                            float(args.router_aux_router_grad_clip),
                        )
                    scaler.step(optimizer)
                    scaler.update()
                    phase_update_step += 1
                ...
                if global_step % args.log_every == 0:
                    row = {
                        ...
                        "dropped_nonfinite_loss": dropped_nonfinite_loss,
                        "dropped_nonfinite_grad": dropped_nonfinite_grad,
                        "logit_scale_exp": float(model.logit_scale.detach().float().exp().cpu()) if hasattr(model, "logit_scale") else float("nan"),
                        **metrics,
                    }
                    ...
                    if args.router_aux_enabled and phase == "1c":
                        append_csv(
                            ...
                            {"phase": phase, "epoch": epoch, "step": global_step, "lr": current_lr, **{
                    ...
                    if args.router_aux_enabled and phase == "1c":
                        router_lr_now = next((float(g["lr"]) for g in optimizer.param_groups if g.get("group_name") == "router"), float("nan"))
                        print(
                            "    [router aux] "
            ...
            summary = {
                ...
                "lr": current_lr,
                "optimizer_steps": optimizer_step_count,
                "phase_optimizer_step": phase_update_step,
                "dropped_nonfinite_loss": dropped_nonfinite_loss,
                "dropped_nonfinite_grad": dropped_nonfinite_grad,
                "logit_scale_exp": float(model.logit_scale.detach().float().exp().cpu()) if hasattr(model, "logit_scale") else float("nan"),
            ...
            # A compact PIECES checkpoint is not sufficient once the backbone moves.
            # Whole-model epoch exports use the exact same wrapped state_dict format
            # as --export_merged_final, so any epoch can be loaded through the normal
            # merged-checkpoint loader without combining it with compact PIECES state.
            # Keep only rolling FULL `last` + FULL `best` by default. Epoch-numbered
            # full checkpoints are huge and are now strictly opt-in. Compact PIECES
            # best/last checkpoints remain independent of this setting.
            save_full_epochs = args.save_full_every_epoch
            if args.all_weights_grad:
                export_merged(
                    model,
                    out_dir / f"phase_{phase}_last_merged_state_dict.pt",
                )
            ...
            if save_full_epochs:
                export_merged(
                    model,
                    out_dir / f"phase_{phase}_epoch_{epoch:03d}_merged_state_dict.pt",
                )
            is_better = (epoch == 1) or (
                math.isfinite(score)
                and (not math.isfinite(best_score) or score < best_score)
            )
            if is_better:
                best_score = score
            ...
            if args.all_weights_grad:
                save_training_diagnostic_plots(out_dir)
        ...
        # Continue from the best checkpoint of this phase. When all weights are
        # trainable, restore the complete best model rather than combining the last
        # backbone with the best compact PIECES state (a subtle Frankenstein bug).
        if args.all_weights_grad:
            best_full_path = out_dir / f"phase_{phase}_best_merged_state_dict.pt"
            if not best_full_path.exists():
                raise FileNotFoundError(
                    f"Missing full best checkpoint for all-weights training: {best_full_path}"
                )
            load_full_model_checkpoint(model, best_full_path)
        else:
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--save_every_epoch",
        action="store_true",
        help="Save compact PIECES checkpoint phase_<phase>_epoch_NNN.pt after every epoch.",
    )
    parser.add_argument(
        "--save_full_every_epoch",
        action="store_true",
        help=(
            "Save the complete merged model state_dict after every epoch as "
            "phase_<phase>_epoch_NNN_merged_state_dict.pt. This is automatically "
            "enabled by --all_weights_grad because compact PIECES checkpoints are "
            "insufficient when the CLIP backbone is trainable."
        ),
    )
    parser.add_argument(
        "--branch_config",
    ...
    parser.add_argument(
        "--all_weights_grad", action="store_true",
        help="Train every CLIP and implant parameter, use AdaBelief, skip gradient clipping, and enable whole-model gradient instrumentation.",
    )
    parser.add_argument("--all_weights_lr", type=float, default=1.0e-6)
    parser.add_argument("--router_aux_all_weights_peak_lr", type=float, default=5.0e-5)
    parser.add_argument("--router_aux_all_weights_floor_lr", type=float, default=5.0e-6)
    parser.add_argument("--router_aux_all_weights_warmup_start_lr", type=float, default=1.0e-6)
    parser.add_argument("--all_weights_eps", type=float, default=1.0e-16)
    parser.add_argument("--all_weights_beta1", type=float, default=0.9)
    parser.add_argument("--all_weights_beta2", type=float, default=0.998)
    parser.add_argument("--all_weights_weight_decay", type=float, default=1.0e-3)
    parser.add_argument("--sched_warmup_batches", type=int, default=200)
    parser.add_argument("--sched_peak_lr", type=float, default=3.0e-6)
    parser.add_argument("--sched_floor_lr", type=float, default=1.0e-7)
    parser.add_argument("--sched_warmup_start_lr", type=float, default=1.0e-8)
    parser.add_argument("--logit_scale_max", type=float, default=100.0)
    parser.add_argument("--grad_log_every", type=int, default=25)
    parser.add_argument("--grad_topk", type=int, default=50)
