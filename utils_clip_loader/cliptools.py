...
class DirectFeatureActivationLoss(InvLoss):
    """Maximize one feature returned directly by a differentiable feature driver.

    ``feature_model(x)`` must return a tensor whose final dimension is the
    feature/neuron dimension.  All preceding dimensions are treated as batch,
    token, patch, or candidate axes.  This is useful for functional activations
    such as ``F.gelu(linear(...))`` that do not exist as standalone modules and
    therefore cannot be selected by the historical QuickGELU hook holder.
    """
    ...
    def loss(self, x: torch.Tensor) -> torch.Tensor:
        activations = self.feature_model(x)
        if not torch.is_tensor(activations):
            raise TypeError(
                "DirectFeatureActivationLoss expected feature_model(x) to return "
                f"a tensor, got {type(activations).__name__}"
            )
        if activations.ndim < 2:
            raise ValueError(
                "Feature activations need a final feature dimension and at least "
                f"one sample dimension, got shape {tuple(activations.shape)}"
            )
        width = int(activations.shape[-1])
        if self.f < 0 or self.f >= width:
            raise IndexError(f"Feature {self.f} is outside activation width {width}")

        values = activations[..., self.f]
        if self.pool == "mean":
            score = values.mean()
        elif self.pool == "max":
            # One spatial/candidate maximum per repeated augmentation, then mean.
            score = values.reshape(values.shape[0], -1).amax(dim=1).mean()
        elif self.pool == "topk":
            flat = values.reshape(values.shape[0], -1)
            k = self.topk if self.topk > 0 else max(1, flat.shape[1] // 16)
            k = min(int(k), int(flat.shape[1]))
            score = flat.topk(k=k, dim=1).values.mean()
        else:
            raise ValueError(
                f"Unknown feature pooling mode {self.pool!r}; use mean, max, or topk"
            )
        return -score
