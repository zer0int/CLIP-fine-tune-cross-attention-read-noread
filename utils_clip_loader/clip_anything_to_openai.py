...
# - Explicit local .pt / .pth:
#     * trusted full model pickle -> reuse the loaded OpenAI-style module
#     * state_dict -> detect OpenAI vs HF -> convert if needed
# - Local .safetensors -> detect OpenAI vs HF -> convert if needed
# - HF Hub repo id (or local HF folder):
#     * safetensors-only by default; never executes remote model code
# - Converted RN / full PIECES exports:
#     * reconstructs their custom tensors in the bundled x-attention-aware oaiclip
#     * standalone RN sidecars can be transplanted into an explicit HF backbone
#
# Notes:
...
def _looks_like_local_path(s: str) -> bool:
    ...
    This avoids accidentally feeding 'foo/bar.pt' into HF repo-id validation.
    """
...
def _load_local_hf_config(folder: str) -> Dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"HF config must be a JSON object: {path}")
    return value


def _find_rn_sidecar(folder: str) -> Optional[str]:
    path = os.path.join(folder, "read_null_token.safetensors")
    return path if os.path.isfile(path) else None


def _is_standalone_rn_state(state_dict: Dict[str, torch.Tensor]) -> bool:
    keys = set(state_dict)
    return bool(keys & {"read_null_token", "visual.read_null_token"}) and not any(
        key.endswith(
            (
...
def _load_rn_sidecar(path: str) -> Tuple[torch.Tensor, Dict[str, str]]:
    try:
        from safetensors import safe_open
    except Exception as error:
        raise ImportError(
            "safetensors is required to load the standalone RN token"
        ) from error
    with safe_open(path, framework="pt", device="cpu") as handle:
        keys = list(handle.keys())
        if "read_null_token" not in keys:
            raise KeyError(f"{path} does not contain 'read_null_token'")
        token = handle.get_tensor("read_null_token").detach().cpu().contiguous()
        metadata = dict(handle.metadata() or {})
    return token.reshape(-1), metadata


def _detect_model_family_from_state(
    ...
    # Parameters are authoritative; model_type is only a fallback for an
    # unusual incomplete state dict. This also lets a stock CLIP config plus
    # an RN sidecar resolve correctly without manual type declarations.
    if any(
        key.startswith("read_implant.source_head.")
        or key.startswith("read_implant.trust_router.")
        or key.startswith("read_implant.read_bridge.")
        or key.startswith("read_implant.orthographic_bridge.")
        or key.startswith("read_implant.presence_pool.")
        or key.startswith("read_implant.glyph_head.")
        for key in keys
    ):
        return "full_xattn"
    if any(
        key in {"content_tap_logits", "read_implant.content_tap_logits"}
        or key.startswith("content_pool.")
        or key.startswith("read_implant.content_pool.")
        for key in keys
    ):
        return "rn_correction"
    if any(
        key in {"read_null_token", "visual.read_null_token"} for key in keys
    ):
        return "rn_token"
    if model_type == "xattn_clip":
        return "full_xattn"
    if model_type == "rn_clip":
        return "rn_correction"
    return "vanilla"


def _detect_model_family_from_model(model: torch.nn.Module) -> str:
    state = model.state_dict()
    return _detect_model_family_from_state(state)


def _restore_fp32_custom_islands(model: torch.nn.Module) -> None:
    """Keep PIECES bridge/router math in FP32 without widening CLIP itself.

    State-dict reconstruction already applies this policy in ``build_model``.
    Trusted full-model pickles bypass that constructor, however, and may store
    the entire module—including ``read_implant``—in FP16.  The FP32 forward
    decorators promote bridge inputs, so the corresponding parameters must be
    promoted as well.
    """
    read_implant = getattr(model, "read_implant", None)
    if isinstance(read_implant, torch.nn.Module):
        read_implant.float()

    # These learned control embeddings are part of the custom PIECES island,
    # not the frozen OpenAI CLIP text-transformer backbone.
...
def _attach_load_info(model: torch.nn.Module, info: ClipLoadInfo) -> None:
    _restore_fp32_custom_islands(model)
    model._clip_load_info = info
    model._clip_model_family = info.model_family
    # The x-attention-aware oaiclip class materializes a bridge scaffold even
    # when the incoming checkpoint is a vanilla or RN-token-only backbone. Do
    # not let that fresh scaffold alter ordinary encode_image() calls. It stays
    # present and trainable for callers that are about to fine-tune a bridge.
    model._clip_apply_content_correction_by_default = info.model_family in {
        "full_xattn",
        "rn_correction",
    }


def _preprocess_for_openai_model(clip_module, model: torch.nn.Module) -> Any:
    """Build the standard OpenAI CLIP transform for an already-loaded model."""
    visual = getattr(model, "visual", None)
    n_px = getattr(visual, "input_resolution", None)
    if n_px is None:
        return None
    transform_fn = _get_openai_transform_fn(clip_module)
    return (
        transform_fn(int(n_px))
        if callable(transform_fn)
        else _build_fallback_transform(int(n_px))
    )


def _update_tokenizer_context_from_model(clip_module, model: torch.nn.Module) -> None:
    """Keep oaiclip.tokenize() aligned with a loaded checkpoint's context size."""
    setter = getattr(clip_module, "_set_default_context_length_from_model", None)
    if callable(setter):
        setter(model)
    context_length = getattr(model, "context_length", None)
    if context_length is not None:
        _set_tokenize_default_context_length(clip_module, int(context_length))


def _load_local_pickle(path: str) -> Any:
    """Load an explicitly supplied local pickle checkpoint.

    Local pickle loading is intentionally supported for checkpoints the caller
    trusts. Hub downloads remain safetensors-only unless explicitly opted in.
    """
    try:
...
def load_openai_clip_anything(
    ...
    reuse_full_model_pickle: bool = True,
) -> Tuple[torch.nn.Module, Any, ClipLoadInfo]:

    architecture_overrides: Dict[str, Any] = {}
    if read_attention_architecture is not None:
        architecture_overrides["read_attention_architecture"] = str(
            read_attention_architecture
        )
    if read_null_enabled is not None:
        architecture_overrides["read_null_enabled"] = bool(read_null_enabled)
    if read_null_insert_block is not None:
        architecture_overrides["read_null_insert_block"] = int(
    ...
    if model_or_path in available:
        load_kwargs: Dict[str, Any] = {"device": device, "jit": jit}
        try:
            load_signature = inspect.signature(clip_module.load)
            accepts_kwargs = any(
                parameter.kind == inspect.Parameter.VAR_KEYWORD
                for parameter in load_signature.parameters.values()
            )
            for key, value in architecture_overrides.items():
                if accepts_kwargs or key in load_signature.parameters:
                    load_kwargs[key] = value
        except (TypeError, ValueError):
            pass
        model, preprocess = clip_module.load(model_or_path, **load_kwargs)
        info = ClipLoadInfo(
        ...
        info.model_family = _detect_model_family_from_model(model)
        _attach_load_info(model, info)
    ...
    if os.path.exists(model_or_path):
        if os.path.isdir(model_or_path):
            hf_config = _load_local_hf_config(model_or_path)
            hf_config.update(architecture_overrides)
            rn_sidecar = _find_rn_sidecar(model_or_path)
            try:
                state_dict, resolved = _load_state_dict_from_hf_folder(
                    model_or_path,
                    allow_unsafe_pickle=allow_unsafe_hf_pickle,
                )
            except FileNotFoundError:
                if rn_sidecar is None:
                    raise
                if not base_model_or_path:
                    raise ClipAnythingLoadError(
                        "Standalone RN-token folders require base_model_or_path. "
                        "Example: base_model_or_path='zer0int/CLIP-GmP-ViT-L-14'."
                    ) from None
                state_dict, base_info = resolve_to_openai_state_dict(
                    base_model_or_path,
                    cache_dir=cache_dir,
                    revision=revision,
                ...
                resolved = rn_sidecar
                hf_config = dict(base_info.config)
                hf_config["model_type"] = "rn_token_sidecar"
                hf_config.update(architecture_overrides)
            if rn_sidecar is not None:
                token, metadata = _load_rn_sidecar(rn_sidecar)
                state_dict = dict(state_dict)
                state_dict["visual.read_null_token"] = token
                if "visual.read_null_insert_block_config" not in state_dict:
                    insert_block = int(
                        metadata.get(
                            "read_null_insert_block",
                            hf_config.get("read_null_insert_block", 20),
                        )
                    )
                    state_dict["visual.read_null_insert_block_config"] = torch.tensor(
                        insert_block, dtype=torch.long
                    )
            return _instantiate_and_load_openai_clip_from_state_dict(
                ...
                hf_config=hf_config,
                base_model_or_path=base_model_or_path,
            )

        ext = os.path.splitext(model_or_path)[1].lower()
        if ext in (".pt", ".pth"):
            obj = _load_local_pickle(model_or_path)
            # Full model pickle passthrough
            if _looks_like_full_model_pickle(obj) and reuse_full_model_pickle:
                # The path was explicitly supplied by the caller, so unpickling is
                # intentional. Reuse this object instead of asking clip.load() to
                # deserialize the multi-GB checkpoint a second time.
                model = obj.to(device)
                if str(device) == "cpu":
                    model.float()
                model.eval()
                preprocess = _preprocess_for_openai_model(clip_module, model)
                _update_tokenizer_context_from_model(clip_module, model)
                info = ClipLoadInfo(
                    source_kind="openai_pickle",
                    resolved_path=model_or_path,
                    detected_format="openai",
                    inferred_openai_model=None,
                    inferred_signature={"note": "loaded trusted full model pickle"},
                )
                info.model_family = _detect_model_family_from_model(model)
                _attach_load_info(model, info)
                return model, preprocess, info
            ...
            return _instantiate_and_load_openai_clip_from_state_dict(
                ...
                strict=strict,
                resolved_path=model_or_path,
                hf_config=architecture_overrides,
            )
    ...
    hf_config = _load_hf_config_from_repo(
        model_or_path, cache_dir=cache_dir, revision=revision
    )
    hf_config.update(architecture_overrides)
    if _is_standalone_rn_state(state_dict):
        if not base_model_or_path:
            raise ClipAnythingLoadError(
                "Standalone RN-token repositories require base_model_or_path."
            )
        token = state_dict.get(
            "read_null_token", state_dict.get("visual.read_null_token")
        )
        base_state, base_info = resolve_to_openai_state_dict(
            base_model_or_path,
            cache_dir=cache_dir,
            revision=revision,
            allow_unsafe_hf_pickle=allow_unsafe_hf_pickle,
        ...
        state_dict["visual.read_null_token"] = token.reshape(-1)
        state_dict["visual.read_null_insert_block_config"] = torch.tensor(
            int(hf_config.get("read_null_insert_block", 20)), dtype=torch.long
        )
        merged_config = dict(base_info.config)
        merged_config.update(hf_config)
        merged_config.update(architecture_overrides)
        merged_config["model_type"] = "rn_token_sidecar"
...
def resolve_to_openai_state_dict(
    ...
    if os.path.exists(model_or_path):
        if os.path.isdir(model_or_path):
            hf_config = _load_local_hf_config(model_or_path)
            sd, resolved = _load_state_dict_from_hf_folder(
                model_or_path,
                allow_unsafe_pickle=allow_unsafe_hf_pickle,
            )
        else:
        ...
        try:
            sd, resolved = _load_state_dict_from_hf_repo(
                model_or_path,
                cache_dir=cache_dir,
                revision=revision,
                allow_unsafe_pickle=allow_unsafe_hf_pickle,
    ...
    openai_sd, info = _convert_any_state_dict_to_openai(
        sd, resolved_path=resolved, hf_config=hf_config
    )
...
def _load_state_dict_from_hf_repo(
    ...
    preferred = ["model.safetensors.index.json", "model.safetensors"]
    if allow_unsafe_pickle:
        preferred += ["pytorch_model.bin.index.json", "pytorch_model.bin"]
    ...
    if picked is None:
        ...
        if picked is None and allow_unsafe_pickle:
            picked = (bin_indexes[0] if bin_indexes else None) \
                  or (bin_singles[0] if bin_singles else None)

    if picked is None:
        unsafe_hint = (
            " Set allow_unsafe_hf_pickle=True only if you trust this repository."
            if any(name.endswith((".bin", ".bin.index.json")) for name in files)
            else ""
        )
        raise ClipAnythingLoadError(Fore.RED +
            f"\nNo safetensors weights found in repo '{repo_id}'." +
            unsafe_hint + Style.RESET_ALL
        )
    ...
    if picked.endswith(".bin.index.json"):
        ...
        for shard in shard_files:
            shard_path = _dl(shard)
            shard_obj = torch.load(shard_path, map_location="cpu", weights_only=True)
...
def _load_state_dict_from_hf_folder(
    folder: str,
    allow_unsafe_pickle: bool = False,
) -> Tuple[Dict[str, torch.Tensor], str]:
    """
    ...
    if os.path.isfile(bin_path):
        if not allow_unsafe_pickle:
            raise ClipAnythingLoadError(
                "Local HF folder contains only pytorch_model.bin. "
                "Use safetensors, or set allow_unsafe_hf_pickle=True for a trusted source."
            )
    ...
    if os.path.isfile(bin_index):
        if not allow_unsafe_pickle:
            raise ClipAnythingLoadError(
                "Local HF folder contains only sharded PyTorch .bin weights. "
                "Use safetensors, or set allow_unsafe_hf_pickle=True for a trusted source."
            )
    ...
    for fn in os.listdir(folder):
        if fn == "read_null_token.safetensors":
            continue
...
def _load_sharded_bin(folder: str, index_json_path: str) -> Dict[str, torch.Tensor]:
    ...
    for shard in shards:
        shard_path = os.path.join(folder, shard)
        shard_sd = torch.load(shard_path, map_location="cpu", weights_only=True)
...
def _convert_any_state_dict_to_openai(
    ...
    config = dict(hf_config or {})
    family = _detect_model_family_from_state(sd, config)
    ...
    if detected == "hf":
        ...
        if "visual.read_null_token" in openai_sd:
            insert_block = int(config.get("read_null_insert_block", 20))
            openai_sd.setdefault(
                "visual.read_null_insert_block_config",
                torch.tensor(insert_block, dtype=torch.long),
            )
...
def _convert_hf_clip_state_dict_to_openai(hf_sd: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    # Custom RN/PIECES tensors intentionally retain OpenAI-side names in the
    # HF export.  The only structural changes are the RN parameter location
    # and the correction-only model's shortened content-pool prefix.
    if "visual.read_null_token" in hf_sd:
        out["visual.read_null_token"] = hf_sd["visual.read_null_token"]
    elif "read_null_token" in hf_sd:
        out["visual.read_null_token"] = hf_sd["read_null_token"]

    if "visual.read_null_insert_block_config" in hf_sd:
        out["visual.read_null_insert_block_config"] = hf_sd[
            "visual.read_null_insert_block_config"
        ]
    ...
    for key, value in hf_sd.items():
        if key.startswith("read_implant."):
            out[key] = value
        elif key.startswith("content_pool."):
            out["read_implant." + key] = value
        elif key == "content_tap_logits":
            out["read_implant.content_tap_logits"] = value
        elif key in {"hard_text_embedding", "null_text_embedding"}:
            out[key] = value
...
def _instantiate_and_load_openai_clip_from_state_dict(
    ...
    info.base_model_or_path = base_model_or_path
    ...
    build_kwargs = {
        "hard_text_token_id": config.get("hard_text_token_id", 49408),
        "no_text_token_id": config.get("no_text_token_id", 49409),
        "any_text_token_id": config.get("any_text_token_id", 49410),
        "null_text_token_id": config.get("null_text_token_id", 49411),
        "eot_token_id": config.get("eot_token_id", 49407),
        "read_attention_architecture": config.get("read_attention_architecture"),
        "read_null_enabled": config.get(
            "read_null_enabled", "visual.read_null_token" in openai_sd
        ),
        "read_null_insert_block": config.get(
            "read_null_insert_block",
            int(
                openai_sd.get(
                    "visual.read_null_insert_block_config", torch.tensor(20)
                ).item()
            ),
        ),
    }
    signature = inspect.signature(build_model_fn)
    supported = {
        key: value
        for key, value in build_kwargs.items()
        if key in signature.parameters and value is not None
    }
    if info.model_family != "vanilla" and "read_null_enabled" not in signature.parameters:
        raise RuntimeError(
            "This RN/PIECES checkpoint requires the x-attention-aware oaiclip "
            "package shipped with this loader."
        )
    model = build_model_fn(openai_sd, **supported).to(device)
    ...
    threshold = float(config.get("register_norm_threshold", 60.0))
    register_min = int(config.get("register_min", 1))
    register_max = int(config.get("register_max", 0))
    visual = getattr(model, "visual", None)
    setter = getattr(visual, "set_register_mask_config", None)
    if callable(setter):
        setter(threshold, register_min, register_max)
