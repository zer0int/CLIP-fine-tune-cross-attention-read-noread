"""Mixed-precision helpers shared by x-attention training and inference."""
...
def resolve_precision(requested: str, device: torch.device | str) -> ResolvedPrecision:
    ...
    if requested == "auto":
        ...
        if not native_bf16:
            ...
            raise RuntimeError(
                f"Native BF16 is unavailable on {name}; use 'auto', 'fp16', or 'none'"
            )
        resolved = "bf16"
    else:
        resolved = requested
...
def autocast_context(device: torch.device | str, resolved: ResolvedPrecision | str):
    ...
    if isinstance(resolved, str):
        if resolved == "auto":
            ...
            dtype = {"bf16": torch.bfloat16, "fp16": torch.float16, "none": None}[
                resolved
            ]
...
def assert_trainable_parameters_fp32(
    module: torch.nn.Module, *, label: str = "model"
) -> None:
    bad = [
